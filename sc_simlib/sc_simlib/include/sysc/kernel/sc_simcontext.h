...
// 사용자가 선택한 오류주입 시나리오 정보를 가지고 있는 구조체을 정의
// XML에서 pasing 한 정보중 실제 오류주입에 사용하는 정보
typedef struct fault_configuration_set
{
	char fault_location[100];	//오류주입 위치 정보가 설정된다.
								// 랜덤일 경우 none값이 그대로 들어간다.

	int fault_time;				// 오류주입 시간 정보가 설정된다.
								// 랜덤(none)경우 임이의 시간이 설정된다.
	
	int fault_type;				//	1. transient
								//	2. permament

	int fault_value;			//	1. stuck-at-0
								//  2. stuck-at-1

}FCS;


typedef struct fault_configuration_data_set
{
	// 공통 오류주입 설정 정보 
	bool fault_enable;				// 오류주입 실험 여부를 확인
	int fault_config_type;	
									// 1 => selected primary
									// 2 => random primary
									// 3 => selected local compound
									// 4 => selected local compound

	int compound_num;				// compound가 있을 때만 설정 
									// 그 외에는 1

	int current_compound;			// compound 오류주입 일 경우 현재 오류주입 실행 시나리오 번호 


	// 개별 오류 시나리오 정보
	// multiple fault이 최대 설정치는 5개로 제한한다. 
	FCS Data[10];


}FCDS;
// multiple permenent 기능을 추가하기 위한 Permanent List를 관리하기 위한 구조체 
typedef struct Permanent_List
{

	char name[10][50];	// 총 5개의 permanent signal 값을 저장 할 수 있다.
...
//[Simcontext_FCDS.current_compound]
//////////////////////////////////////////////////////////////////
...
public:
	...
	bool fault_set;
	char per_signal[50];
	bool fault_location_set;
	int fault_signal_sel;
	unsigned fault_data;
	int rand_signal_select;

	int signal_bindo;				///////////////////////test parameter1
	int signal_write;				///////////////////////test parameter2
	...
	FCDS Simcontext_FCDS;

	Per_List permanent_l;
