...
#define		STATE_F_Location	1000
#define		STATE_F_Time		2000
#define		STATE_F_Type		3000
#define		STATE_F_Value		4000
#define		STATE_Unknown		100


typedef struct Fault_secnario_set
{
	char F_loc[100];
	char F_time[20];
	char F_type[30];
...
sc_simcontext::init()
{
	printf("********************************** \n");
	printf("*                                * \n");
	printf("*  SyFI Version 1.1 working...   * \n");
	printf("*                                * \n");
	printf("*  Date 2010. 10. 13             * \n");
	printf("*  work XML Paser porting        * \n");
	printf("*                                * \n");
	printf("********************************** \n");
	...
	IrrXMLReader* xml = createIrrXMLReader("fault_configuration.xml");

	// d; '� vü ��
	F_Config FSS_DATA[10];				// XML parsing ���'� ��...

	F_Config Selected_Fault_Secnario;

	int current_config_index = -1;		// 迭 �� 0����ؾ��ϱ�!!!
	int current_pasing_state =0;
	std::string element_data;

	char temp_sel_secnario[10];
	char temp_fault_set[10];
	int sel_secnario = 0;
	//int fault_set = 0;
	...
	while(xml && xml->read())
	{
		...
		case EXN_TEXT:
				...
				element_data = xml->getNodeData();

				// �� element   d ������
				if( current_pasing_state == STATE_F_Location)
				{
						strcpy(FSS_DATA[current_config_index].FS[Inc_FS_NUM].F_loc, element_data.c_str());
						// ����
						current_pasing_state = STATE_Unknown;
				}
				else if( current_pasing_state == STATE_F_Time)
				{
					strcpy(FSS_DATA[current_config_index].FS[Inc_FS_NUM].F_time, element_data.c_str());
					// ����
					current_pasing_state = STATE_Unknown;

				}
				else if( current_pasing_state == STATE_F_Type)
				{
					strcpy(FSS_DATA[current_config_index].FS[Inc_FS_NUM].F_type, element_data.c_str());
					// ����
					
					current_pasing_state = STATE_Unknown;

				}
				else if( current_pasing_state == STATE_F_Value)
				{
					strcpy(FSS_DATA[current_config_index].FS[Inc_FS_NUM].F_value, element_data.c_str());
					// ����
					current_pasing_state = STATE_Unknown;
				}
				else
			...
			break;
		case EXN_ELEMENT:			// �� element� ; Ȯ� � ���
			{
				if(!strcmp("Fault_experiment", xml->getNodeName()))
				{
					// 7�� ο�7�� ó8 Ȯ� ��
					strcpy(temp_fault_set, xml->getAttributeValue("fault_enable")) ;
					strcpy(temp_sel_secnario, xml->getAttributeValue("select_secnario")) ;

					if(strcmp(temp_fault_set, "yes") == 0)
					{
						Simcontext_FCDS.fault_enable = true;
					}
					else
					{
						Simcontext_FCDS.fault_enable = false;
					}
					
					sel_secnario =atoi(temp_sel_secnario);
					printf("&&&&&&&&&&&&&&&&&&&& %d\n", sel_secnario);


				}
				else if (!strcmp("Fault_config", xml->getNodeName()))
				{
					current_config_index++;	// d� Ǿ���7�� ó8 ���'� �
					Inc_FS_NUM =-1;
					char temp_num[5];

					strcpy(FSS_DATA[current_config_index].FSS_type , xml->getAttributeValue("type")) ;
					strcpy(temp_num , xml->getAttributeValue("num")) ;
					FSS_DATA[current_config_index].FS_NUM = atoi(temp_num);
					
				}
				else if(!strcmp("F_Location", xml->getNodeName()))
				{
					Inc_FS_NUM++;		// selected comppound 7�� �� �  7�� 'ġ� �� '� �

					current_pasing_state = STATE_F_Location;
					
				}
				else if(!strcmp("F_Time", xml->getNodeName()))
				{
					current_pasing_state = STATE_F_Time;

				}
				else if(!strcmp("F_Type", xml->getNodeName()))
				{
					current_pasing_state = STATE_F_Type;
					
					// noting attribute
				
				}
				else if(!strcmp("F_Value", xml->getNodeName()))
				{
					current_pasing_state = STATE_F_Value;
	...
	// �� Ǽ����ó
	if( sel_secnario-1 > current_config_index)
	{
		printf("x��� ʴ�ó0 �Ǿ�ùķ�� f ~��ϴ�\n");
		return ;

	}

	strcpy(Selected_Fault_Secnario.FSS_type, FSS_DATA[sel_secnario-1].FSS_type);
	Selected_Fault_Secnario.FS_NUM = FSS_DATA[sel_secnario-1].FS_NUM;

	if(!(strcmp(Selected_Fault_Secnario.FSS_type, "Selected primary")))
	{
		Simcontext_FCDS.fault_config_type = 1;
		Simcontext_FCDS.compound_num = 1;
		fault_location_set = true;
	}
	if(!(strcmp(Selected_Fault_Secnario.FSS_type, "Random Primary")))
	{
		Simcontext_FCDS.fault_config_type = 2;
		Simcontext_FCDS.compound_num = 1;
		fault_location_set = false;
	}
	if(!(strcmp(Selected_Fault_Secnario.FSS_type, "Selected Local compound")))
	{
		Simcontext_FCDS.fault_config_type = 3;
		Simcontext_FCDS.compound_num = Selected_Fault_Secnario.FS_NUM;
		fault_location_set = true;

	}
	if(!(strcmp(Selected_Fault_Secnario.FSS_type, "Random Local compound")))
	{
		Simcontext_FCDS.fault_config_type = 4;
		Simcontext_FCDS.compound_num = Selected_Fault_Secnario.FS_NUM;
		fault_location_set = false;
	...
	Simcontext_FCDS.current_compound = 0;		// compound 7� � �� 7�� ȣ 

	for(int j=0; j< Selected_Fault_Secnario.FS_NUM; j++)
	{
		strcpy(Selected_Fault_Secnario.FS[j].F_loc, FSS_DATA[sel_secnario-1].FS[j].F_loc);
		strcpy(Selected_Fault_Secnario.FS[j].F_time, FSS_DATA[sel_secnario-1].FS[j].F_time);
		strcpy(Selected_Fault_Secnario.FS[j].F_type, FSS_DATA[sel_secnario-1].FS[j].F_type);
		strcpy(Selected_Fault_Secnario.FS[j].F_value, FSS_DATA[sel_secnario-1].FS[j].F_value);

		/// ó�d � 
		strcpy(Simcontext_FCDS.Data[j].fault_location,  Selected_Fault_Secnario.FS[j].F_loc);
		

		if(!(strcmp(Selected_Fault_Secnario.FS[j].F_time, "none")))
		{
			Simcontext_FCDS.Data[j].fault_time = 0;
		}
		else
		{
			Simcontext_FCDS.Data[j].fault_time = atoi(Selected_Fault_Secnario.FS[j].F_time);
		}

		if(!(strcmp(Selected_Fault_Secnario.FS[j].F_type, "Transient")))
		{
			Simcontext_FCDS.Data[j].fault_type  = 1;

		}
		else if(!(strcmp(Selected_Fault_Secnario.FS[j].F_type, "Permanent")))
		{
			Simcontext_FCDS.Data[j].fault_type  = 2;
		}
		else
		{
			
			Simcontext_FCDS.Data[j].fault_type  = 0;
			;
		}

		if(!(strcmp(Selected_Fault_Secnario.FS[j].F_value, "Stuck-at-0")))
		{
			Simcontext_FCDS.Data[j].fault_value = 1;

		}
		else if(!(strcmp(Selected_Fault_Secnario.FS[j].F_value, "Stuck-at-1")))
		{
			Simcontext_FCDS.Data[j].fault_value = 2;
		}
		else
		{
			Simcontext_FCDS.Data[j].fault_value = 0;
		}
		
	}

		
	printf("Selected Fault Injection Secnario\n");
	
	printf(" Type is %s, num is %d\n", Selected_Fault_Secnario.FSS_type, Selected_Fault_Secnario.FS_NUM);

	for(int j=0; j< Selected_Fault_Secnario.FS_NUM; j++)
	{
		
		printf("%s: %s\n", Selected_Fault_Secnario.FS[j].F_loc, Simcontext_FCDS.Data[j].fault_location);	
		printf("%s: %d\n", Selected_Fault_Secnario.FS[j].F_time, Simcontext_FCDS.Data[j].fault_time);	
		printf("%s: %d\n", Selected_Fault_Secnario.FS[j].F_type, Simcontext_FCDS.Data[j].fault_type);	
		printf("%s: %d\n", Selected_Fault_Secnario.FS[j].F_value, Simcontext_FCDS.Data[j].fault_value);	
	}
	...
	permanent_l.current_list_num =0;
	permanent_l.enable =0;
	...
	while( true ) {
		...
		if( thread_h != 0 ) {
			...
			if(crunch_set == false)
			{
				...
				{
					// selected primariy 7�� 
					if( Simcontext_FCDS.fault_config_type == 1 || Simcontext_FCDS.fault_config_type == 3)		// deterministic � 7�� 
					{
						//	printf("AAAAAAAAAAAAAAAAAAAAAAA\n");
							crunch_set = true;		// �� �Ǿ��̻�ȵ� ð�̴�
							fault_set = true;
							//select_signal  = rand() % signal_write;
					}
					else
					{
						
						//printf("AAAAAAAAAAAAAAAAAAAAAAA  %d   %d \n", fault_signal_sel, signal_write);
						// random location 7�� 
						if(fault_signal_sel ==0)
						{
							if(signal_write != 0)
							{
								//printf("fault injection ready~!~!!!\n");
								crunch_set = true;		// �� �Ǿ��̻�ȵ� ð�̴�
								fault_set = true;
								select_signal  = rand() % signal_write;
...
//////////////////////////////////////////////////
	// 7�� ð� random d��7��; d��
	// 10 18� �� ��7 ��ð d � Ǿ���
	// Data �Ǵ�÷��compound 7� � � primary� � ׳�08�d 
	if(Simcontext_FCDS.fault_enable == true)
	{
		if(Simcontext_FCDS.Data[0].fault_time == 0)
		{
		
			// ramdom time - premary & compound
			for(int x=0; x < Simcontext_FCDS.compound_num; x++)
			{
				
				while(fault_time[x] <= 5000000 || fault_time[x] >= 10000000)
				{
					fault_time[x] =0;
					...
					fault_time[x] = data1 * data2 * data3 * data4 * data5;
				
				}
				if(Simcontext_FCDS.fault_config_type != 4)
					printf("fault injection time->upVer : %d \n", fault_time[x]);
			}
			...
			if(Simcontext_FCDS.fault_config_type == 4)
			{
				...
				int temp_time =0;
				for(int y=0; y < Simcontext_FCDS.compound_num-1; y++)
				{
					for(int k=y+1; k< Simcontext_FCDS.compound_num; k++)
					{	
						//printf(" %d    %d \n", fault_time[y], fault_time[k]);

						if(	fault_time[y] >	fault_time[k])
						{
							temp_time = fault_time[y];
							fault_time[y] = fault_time[k];
							fault_time[k] = temp_time;
				...
				for(int j=0; j < Simcontext_FCDS.compound_num; j++)
				{
		...
		else 
		{
			...
			// deterministic � 7 ���� 
			if(Simcontext_FCDS.fault_config_type == 1 || Simcontext_FCDS.fault_config_type == 2)
			{
				fault_time[Simcontext_FCDS.current_compound] = Simcontext_FCDS.Data[0].fault_time;		// deterministic� 7�� � single fault 
			}
			else if(Simcontext_FCDS.fault_config_type == 3 || Simcontext_FCDS.fault_config_type == 4)
			{
				for(int i=0; i< Simcontext_FCDS.compound_num; i++)
				{
					fault_time[i] = Simcontext_FCDS.Data[i].fault_time;
				}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
	if((fault_time[Simcontext_FCDS.current_compound] == t.m_value || 
		fault_time[Simcontext_FCDS.current_compound] < t.m_value) && set == false)
	{
		//fault_set = true;
		// 7�� 'ġ random8�dǾ���� .....
		set = true;
		...
		if(Simcontext_FCDS.fault_config_type == 1)		//  selected primary
		{


		}
		else if(Simcontext_FCDS.fault_config_type == 2)	// Random primary
		{
			
			// 7�� 'ġ ī��.. 
			for(int i =0 ; i <10; i++)
			{
				fault_signal_sel = rand() % 10;		// �Ǿ���module 10 ��� �
				

			}
			//printf("aaaaaaaaaaaaaaaaaaaaaa  %d \n", fault_signal_sel);
		}
		else if(Simcontext_FCDS.fault_config_type == 3)	// selected local compound(̱�)
		{
			//printf("SSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSo\n");
	
		}
		else if(Simcontext_FCDS.fault_config_type == 4)	// random local compound(̱�)
		{
